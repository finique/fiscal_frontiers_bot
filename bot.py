...
def perform_analysis_description(message):
    ...
    if user_id not in user_tickers or user_tickers[user_id] is None:
        ...
        return
    ticker = user_tickers[user_id]
    analyze_news(ticker, message.chat.id)
...
def analyze_news(ticker, chat_id):
    ticker = ticker.strip().upper()
    news = get_news(ticker)

    if len(news)>1 :
        bot.send_message(chat_id, f"{news}")
        del news
    else:
        bot.send_message(chat_id, "Sorry, there was a problem with the news.")
